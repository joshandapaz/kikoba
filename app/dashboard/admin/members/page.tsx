  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        ...
        <p className="page-subtitle">Dhibiti wanachama wa kikundi na vyeo vyao</p>
      </div>
      ...
      <div className="page-content">
        ...
        <div className="card" style={{ marginBottom: 32, padding: '32px' }}>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 18, fontWeight: 800, marginBottom: 20, letterSpacing: '-0.5px' }}>
            <UserPlus size={22} color="#FFF" /> Ongeza Mwanachama
          </h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 24, fontWeight: 500 }}>
            Ingiza ID ya kipekee ya mwanachama ili kumwongeza moja kwa moja kwenye kikundi.
          </p>
          ...
          <form onSubmit={handleAddMember} style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <button type="submit" className="btn-primary" style={{ width: 'auto', padding: '14px 32px' }} disabled={addingMember}>
              {addingMember ? <span className="spinner" /> : <>Ongeza <ArrowRight size={18} /></>}
            </button>
          </form>
        </div>
          ...
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Mwanachama</th>
                  <th>Barua Pepe & Simu</th>
                  <th>Tarehe Kujiunga</th>
                  <th>Cheo</th>
                    ...
                    <tr key={m.id}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <span style={{ fontWeight: 700, fontSize: 15 }}>{m.user.username}</span>
                        </div>
                      </td>
                      <td>
                        <div style={{ fontSize: 14, color: '#FFF', fontWeight: 500 }}>{m.user.email}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>{m.user.phone || '-'}</div>
                      </td>
                      <td style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>{formatDate(m.joinedAt)}</td>
                      <td>
